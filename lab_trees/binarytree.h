...
class BinaryTree
{
  ...
  private:
    ...
    /** 
      ...
      * @param subRoot the current node in recursion
      * @return nothing
      */
    ...
    T max1(const Node* subRoot) const;
    T min1(const Node* subRoot) const;

    void printPaths(const Node* subRoot, vector<T> &v) const;

    void sumDistances(const Node* subRoot, vector<T> &v, int &sum, int pos) const;
    /**
