...
void LPHashTable<K, V>::insert(K const& key, V const& value)
{
     ...
     */
    if(keyExists(key)) //if the key already exists return
        return;

    elems++; //otherwise increase elems
    if(shouldResize()) //resize if needed
    {
        resizeTable();
    }
    //once resize is done, now we add the key
    auto idx = hash(key, size); // we get the location to put the key value in
    pair<K, V> *nw_pair = new pair<K, V>(key, value);

    while(should_probe[idx]) //lets check if need to probe the idx
    {
        idx = (idx + 1) % size; //
    }
