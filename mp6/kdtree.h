...
class KDTree
{
  ...
  private:
     ...
     */
    int partition(int k, int left, int right, int median);
    void swap(int pos1, int pos2);
    void KDCtor(int left, int right, int k);
    void select(int k, int left, int right, int median);
    void nearNeighbor(const Point<Dim>& query, Point<Dim>& retVal, int left, int right, int k) const;
    bool splitChecker(const Point<Dim>& query, Point<Dim> split, Point<Dim> best, int k) const;
