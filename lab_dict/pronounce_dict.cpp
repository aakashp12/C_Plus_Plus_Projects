...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    string a = word1;
    string b = word2;
    std::transform(a.begin(), a.end(), a.begin(), ::toupper); //change word1 to upper case
    std::transform(b.begin(), b.end(), b.begin(), ::toupper); //change word2 to upper case
    ...
    int jk = 0;
    for(auto & key : dict.at(a) )
    {
        if(key[0] == 'A' || key[0] == 'E' || key[0] == 'I' || key[0] == 'O' || key[0] == 'U' || key[0] == 'Y')
            jk++;
    }
    if(jk > 1)
        return false;
    if(dict.at(a) != dict.at(b) )
    {
        return false;
    }
    return true;
