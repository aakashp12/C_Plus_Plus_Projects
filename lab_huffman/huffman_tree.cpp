...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    */
    ...
    while(!singleQueue.empty() || mergeQueue.size() != 1) //while both of the queues have something in them
    {
    	TreeNode* L_node = removeSmallest(singleQueue, mergeQueue);
    	TreeNode* R_node = removeSmallest(singleQueue, mergeQueue);
        TreeNode* parent = new TreeNode(L_node->freq.getFrequency() + R_node->freq.getFrequency());//create new tree node.
    	parent->left = L_node;
    	parent->right = R_node;
	    mergeQueue.push(parent); //   push the node to mergeQueue
    }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
	...
	{
		Frequency a(bfile.getNextByte(), 0);
