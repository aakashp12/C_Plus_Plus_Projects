...
class BTree
{
  ...
  public:
     ...
     * is order 3.if(pruneSize(current) <= numLeaves && pruneSize(current - 1) > numLeaves)
		{
			break;
		}
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    if(elements[0] > val)
        return 0;
    int end = (int)elements.size() - 1; // get the size of this vector;
    int start = 0;
    int current = (end + start) / 2;

    while(start <= end)
    {
        if(val < elements[current])
        {
            end = current - 1;
            current = (start + end) / 2;
        }
    ...
    return start; //the loop breaks when it finds a spot and then you break and have the i value. 
}
