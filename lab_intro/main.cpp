...
PNG brighten(PNG original, int amount)
{
    ...
    for (size_t yi = 0; yi < original.height(); yi++) {
        for (size_t xi = 0; xi < original.width(); xi++) {
            /// Your code here!
		int red1 = original(xi,yi)->red;
		int green1 = original(xi,yi)->green;
		int  blue1 = original(xi,yi)->blue;

		if(red1 + amount > 255)
		{
			original(xi,yi)->red = 255;
		}
		else original(xi,yi)->red = red1 + amount;

		if(green1 + amount > 255)
		{
			original(xi,yi)->green = 255;
		}
		else original(xi,yi)->green = green1 + amount;

		if(blue1 + amount > 255)
		{
			original(xi,yi)->blue = 255;
		}
		else original(xi,yi)->blue = blue1 + amount;        
	}
    }
    return original;
}
...
PNG blendImages(PNG firstImage, PNG secondImage)
{
	...
	for(size_t y = 0; y < firstImage.height(); y++)
	{
		...
		{
			int red = firstImage(x,y)->red;
			int red1 = secondImage(x,y)->red;
			
			int green = firstImage(x,y)->green;
			int green1 = secondImage(x,y)->green;

			int blue = firstImage(x,y)->blue;
			int blue1 = secondImage(x,y)->blue;
			
			firstImage(x,y)->red = (red + red1) / 2;
			firstImage(x,y)->green = (green + green1) / 2;
			firstImage(x,y)->blue = (blue + blue1) / 2;	
