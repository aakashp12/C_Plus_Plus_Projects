...
class AVLTree
{
  ...
  private:
    ...
    /** 
      * Private helper function to find the maximum height
      * @param subRoot->left->height and subRoot->right->height
      */
    //int max(Node* subRoot);

    /**
      * Private helper function to get the IN-ORDER PREDECESSOR
      * @param subtree
       */
    Node* find_P(Node*& subRoot);
    void delete_P(Node*& subRoot);
